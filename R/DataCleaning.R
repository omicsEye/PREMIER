...
Ile_ApoA1 <- ggplot(df, aes(x = Ile, y = ApoA1, color = as.character(Visit), shape = SEX)) +
  geom_point(alpha=.7) +
  labs(
       x = "Ile",
       y = "ApoA1",
       color = "Visit") +
  theme_omicsEye()
ggsave(filename = 'Ile_ApoA1.pdf', plot=Ile_ApoA1, width = 3.6, height = 3, units = "in", dpi = 350)
...
write.table( t(BCAAs),"BCAA.tsv",
             sep = "\t", eol = "\n", quote = F, col.names = NA, row.names = T)
write.table( t(lipoproteins),"lipoproteins.tsv",
             sep = "\t", eol = "\n", quote = F, col.names = NA, row.names = T)
...
metabolites_followup = subset(metabolites_followup, select = -c(`Sample id`, ID, STUDY_ID, VISIT) )

diff_df <- metabolites_followup - metabolites_base


df <- cbind(raw_metadata[rownames(diff_df),],diff_df)
Ile_ApoA1 <- ggplot(df, aes(x = Ile, y = ApoA1, color = as.character(INTERVENTION), shape = SEX)) +
  geom_point(alpha=.7) +
  labs(
    x = "Ile",
    y = "ApoA1",
  ...
  theme_omicsEye()
Ile_ApoA1
ggsave(filename = 'Ile_ApoA1_diff.pdf', plot=Ile_ApoA1, width = 3.6, height = 3, units = "in", dpi = 350)


BCAA_cols <- c("BCAA", "Ile", "Leu", "Val")

diff_BCAA <- diff_df[, BCAA_cols]
diff_lipoproteins <- diff_df[, !colnames(diff_df) %in% BCAA_cols]
write.table( t(diff_BCAA),"diff_BCAA.tsv",
             sep = "\t", eol = "\n", quote = F, col.names = NA, row.names = T)
write.table( t(diff_lipoproteins),"diff_lipoproteins.tsv",
             sep = "\t", eol = "\n", quote = F, col.names = NA, row.names = T)
